...
function DBMBC:ADDON_LOADED(mod)
	...
	for i = 1, GetNumAddOns() do
		if GetAddOnMetadata(i, "X-DBM-BC-AddOn") and not checkEntry(bannedMods, GetAddOnInfo(i)) then
			...
			-- Process the zoneId array for the addon we just added
			local currentAddon = DBM.AddOns[#DBM.AddOns]
			if currentAddon.zoneId then
				for j = #currentAddon.zoneId, 1, -1 do
					local id = tonumber(currentAddon.zoneId[j])
					if id then
						currentAddon.zoneId[j] = id
					else
						table.remove(currentAddon.zoneId, j)
					end
				end
			end
		end

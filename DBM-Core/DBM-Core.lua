...
DBM = {
	Revision = parseCurseDate("20260919180656"), -- hardcoded: bump manually when cutting a release
	DisplayVersion = "1.0.3-ygg", -- hardcoded base version
}
	...
	function DBM:COMBAT_LOG_EVENT_UNFILTERED(timestamp, event, sourceGUID, sourceName, sourceFlags, destGUID, destName, destFlags, extraArg1, extraArg2, extraArg3, extraArg4, extraArg5, extraArg6, extraArg7, extraArg8, extraArg9, extraArg10, extraArg11, extraArg12)
		...
		if noArgTableEvents[event] then
			...
			if eventSub6 == "SPELL_" then
				...
				if event == "UNIT_HEALTH" or event == "SPELL_AURA_REFRESH" or event == "SPELL_AURA_REMOVED" or event == "SPELL_AURA_BROKEN" then
		...
		if not checkEntry(inCombat, mod) then
			...
			if event ~= "TIMER_RECOVERY" then
				...
				if mod.OnCombatStart then
					...
					mod:OnCombatStart(delay or 0, startEvent == "PLAYER_REGEN_DISABLED_AND_MESSAGE" or startEvent == "PLAYER_REGEN_DISABLED" or startEvent == "SPELL_CAST_SUCCESS" or startEvent == "MONSTER_MESSAGE", startEvent == "ENCOUNTER_START")
				end
				--send "C" sync
				event = event or ""
				if not synced then
...
--		["reflect"] = "target",
	...
	function bossModPrototype:NewSpecialWarningSpellOnBoss(spellId, optionDefault, ...) -- Nick bookmark
		return newSpecialWarning(self, "boss", spellId, nil, optionDefault, ...)
	end
	...
	function bossModPrototype:NewSpecialWarningOnBoss(spellId, optionDefault, stacks, ...) -- NICK BOOKMARK
		return newSpecialWarning(self, "boss", spellId, stacks, optionDefault, ...)
	end

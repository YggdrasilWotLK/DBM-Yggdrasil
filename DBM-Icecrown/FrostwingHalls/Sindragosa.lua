...
local timerNextBlisteringCold	= mod:NewCDTimer(75, 70123, nil, nil, nil, 2, nil, DBM_COMMON_L.DEADLY_ICON, true, 2) -- Added "keep" arg
local timerNextIcyGrip			= mod:NewCDTimer(69, 70117, nil, nil, nil, 2, nil, DBM_COMMON_L.DEADLY_ICON, true, 2) --Nick bookmark
...
function mod:OnCombatStart(delay)
	...
	timerNextBlisteringCold:Start(41-delay) --Nick bookmark
	timerNextIcyGrip:Start(35-delay) --Nick bookmark
	...
	elseif spellId == 70126 then
		...
		if self.vb.phase == 2 then--Phase 2 there is only one icon/beacon, don't use sorting method if we don't have to.
			if self.vb.beaconP2Count == 4 or self.vb.beaconP2Count == 8 or self.vb.beaconP2Count == 12 or self.vb.beaconP2Count == 16 then
				timerNextBlisteringCold:Start(19)--Nick bookmark p2 blistering timers
				timerNextIcyGrip:Start(13)--Nick bookmark p2 blistering timers
				timerNextBeacon:Start(19.5, self.vb.beaconP2Count)
			...
			timerNextBeacon:Start(16.5, self.vb.beaconP2Count)
...
function mod:CHAT_MSG_MONSTER_YELL(msg)
	if (msg == L.YellAirphase or msg:find(L.YellAirphase)) or (msg == L.YellAirphaseDem or msg:find(L.YellAirphaseDem)) then
		...
		timerNextBlisteringCold:Start(84)--Not exact anywhere from 80-110seconds after airphase begin
		timerNextIcyGrip:Start(78)--Nick bookmark
	...
	elseif (msg == L.YellPhase2 or msg:find(L.YellPhase2)) or (msg == L.YellPhase2Dem or msg:find(L.YellPhase2Dem)) then
		...
		timerNextBlisteringCold:Start(99)
		timerNextIcyGrip:Start(93)--Nick bookmark
	end
end